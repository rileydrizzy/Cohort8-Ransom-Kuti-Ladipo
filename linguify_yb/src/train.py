...
from linguify_yb.src.dataset.dataset import get_test_dataloader
...
def train(model, optim, loss_func, n_epochs, FILES, batch, device):
    ...
    for epoch in range(n_epochs):
        ...
        loss = mini_batch(model, dataloader, optim, device, loss_func, validation=False)
        ...
        if epoch // 2 == 0:
            with torch.no_grad():
                ...
                val_loss = mini_batch(
                    model, dataloader, optim, device, loss_func, validation=True
                )
...
def main(args):
    ...
    model = ModelLoader().get_model(args.model)
